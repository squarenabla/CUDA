...
unsigned char _xtime(const unsigned char x){
	return ((x<<1) ^ (((x>>7) & 1) * 0x1b));
}

unsigned char _multiply(const unsigned char x, const unsigned char y){
	return (((y & 1) * x) ^ ((y>>1 & 1) * _xtime(x)) ^ ((y>>2 & 1) * _xtime(_xtime(x))) ^ 
		((y>>3 & 1) * _xtime(_xtime(_xtime(x)))) ^ ((y>>4 & 1) * _xtime(_xtime(_xtime(_xtime(x))))));
}

bool AES::encode(unsigned char* data, const unsigned char (&cipher) [BLOCK_SIZE]){
	//if(size != BLOCK_SIZE){
...
bool AES::decode(unsigned char* data, const unsigned char (&cipher) [BLOCK_SIZE]){
	//if(size != BLOCK_SIZE){
	//	printf("%d %d \n", (int)sizeof(data), (int)sizeof(cipher));
	//	return false;
	//}
...
void AES::_invMixColumns(unsigned char *r){
	for(int i=0; i<BLOCK_DEMEN; i++){
		...
		r[i + BLOCK_DEMEN * 0] = _multiply(a, 0x0e) ^ _multiply(b, 0x0b) ^ _multiply(c, 0x0d) ^ _multiply(d, 0x09);
		r[i + BLOCK_DEMEN * 1] = _multiply(a, 0x09) ^ _multiply(b, 0x0e) ^ _multiply(c, 0x0b) ^ _multiply(d, 0x0d);
		r[i + BLOCK_DEMEN * 2] = _multiply(a, 0x0d) ^ _multiply(b, 0x09) ^ _multiply(c, 0x0e) ^ _multiply(d, 0x0b);
		r[i + BLOCK_DEMEN * 3] = _multiply(a, 0x0b) ^ _multiply(b, 0x0d) ^ _multiply(c, 0x09) ^ _multiply(d, 0x0e);
	}
}
